...
# Load the ontology
ontology = owl.get_ontology("data/ontology.owl")
ontology.load()
...
def get_class_leaves(onto_class: owl.entity.ThingClass,
    ...
    if not subclasses or (0 <= max_level <= current_level):
        ...
        for subclass in onto_class.subclasses():
            leaves += get_class_leaves(subclass, max_level=max_level, current_level=current_level + 1, return_depth=return_depth)
...
def check_relation(restriction: Union[owl.entity.ThingClass, owl.class_construct.ClassConstruct], property: owl.prop.ObjectPropertyClass, object: owl.entity.ThingClass) -> bool:
    """
...
def get_class_properties(ontology: owl.namespace.Ontology,
    ...
    with open('data/class_properties.json', 'w') as json_file:
